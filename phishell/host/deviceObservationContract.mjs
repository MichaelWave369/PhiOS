...
export function validateDeviceObservation(o){
  ...
  if(Array.isArray(o.network)) for(const r of o.network) if(!keys(r,["name","type","operState","vendorId","deviceId"])||!str(r.name,128)||!(r.type===null||nni(r.type))||!nullable(r.operState,32)||!nullable(r.vendorId,16)||!nullable(r.deviceId,16)) {e.push("network row");break}
  ...
  if(Array.isArray(o.usb)) for(const r of o.usb) if(!keys(r,["pathId","vendorId","productId","deviceClass","manufacturer","product"])||!str(r.pathId,128)||!str(r.vendorId,16)||!str(r.productId,16)||!nullable(r.deviceClass,16)||!nullable(r.manufacturer,120)||!nullable(r.product,120)) {e.push("usb row");break}
  if(Array.isArray(o.block)) for(const r of o.block) if(!keys(r,["name","sizeBytes","removable","rotational","vendor","model"])||!str(r.name,128)||!(r.sizeBytes===null||(Number.isSafeInteger(r.sizeBytes)&&r.sizeBytes>=0))||typeof r.removable!=="boolean"||!(r.rotational===null||typeof r.rotational==="boolean")||!nullable(r.vendor,80)||!nullable(r.model,120)) {e.push("block row");break}
  if(Array.isArray(o.drm)) for(const r of o.drm) if(!keys(r,["name","vendorId","deviceId"])||!str(r.name,128)||!nullable(r.vendorId,16)||!nullable(r.deviceId,16)) {e.push("drm row");break}
  if(Array.isArray(o.power)) for(const r of o.power) if(!keys(r,["name","type","manufacturer","model"])||!str(r.name,128)||!nullable(r.type,64)||!nullable(r.manufacturer,120)||!nullable(r.model,120)) {e.push("power row");break}
