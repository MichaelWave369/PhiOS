import json
import threading
from contextlib import contextmanager
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer
from pathlib import Path
from typing import Iterator

import pytest

import phios.reality.service as reality_service
from phios.mandala import MandalaStatus
from phios.reality import (
    JsonMixedContractClause,
    LocalHttpObservation,
    LocalHttpObservationError,
    RealityClaim,
    RealityClaimKind,
    RealityVerdict,
    StdlibLoopbackHttpStateProvider,
)
from phios.spine.runtime import PhiOSSpine


class FakeClock:
    def __init__(self) -> None:
        self.now = 0.0
        self.sleeps: list[float] = []

    def monotonic(self) -> float:
        return self.now

    def sleep(self, seconds: float) -> None:
        self.sleeps.append(seconds)
        self.now += seconds

    def advance(self, seconds: float) -> None:
        self.now += seconds


class TimedSequenceProvider:
    name = "timed-sequence-http"

    def __init__(
        self,
        *,
        clock: FakeClock | None,
        items: list[LocalHttpObservation | Exception],
        durations: list[float] | None = None,
    ) -> None:
        self.clock = clock
        self.items = list(items)
        self.durations = durations or [0.0] * len(items)
        self.calls = 0
        self.start_times: list[float] = []

    def observe(
        self,
        *,
        url: str,
        timeout_seconds: float,
        max_body_bytes: int,
    ) -> LocalHttpObservation:
        index = self.calls
        self.calls += 1
        if self.clock is not None:
            self.start_times.append(self.clock.monotonic())
        item = self.items[index]
        if self.clock is not None:
            self.clock.advance(self.durations[index])
        if isinstance(item, Exception):
            raise item
        return item


def _observation(
    index: int,
    *,
    body: bytes | None = None,
    ready: bool = True,
    queue_depth: int = 3,
    status: int = 200,
    truncated: bool = False,
    captured_at_utc: str | None = None,
) -> LocalHttpObservation:
    if body is None:
        body = json.dumps(
            {
                "models": [{"name": "alpha"}],
                "ready": ready,
                "queue_depth": queue_depth,
            },
            separators=(",", ":"),
        ).encode("utf-8")
    return LocalHttpObservation(
        url="http://127.0.0.1:11434/api/state",
        method="GET",
        status_code=status,
        reason="OK",
        headers={"content-type": "application/json"},
        body_sha256=f"{index + 11:064x}",
        body_bytes_observed=len(body),
        body_truncated=truncated,
        body_digest_scope="prefix" if truncated else "full",
        redirect_followed=False,
        timeout_seconds=2.0,
        max_body_bytes=65_536,
        elapsed_ms=2.0 + index,
        provider="timed-sequence-http",
        provider_version="1.0",
        captured_at_utc=(
            captured_at_utc
            if captured_at_utc is not None
            else f"2026-09-18T22:20:0{index}+00:00"
        ),
        body=body,
    )


def _mixed_clauses() -> tuple[JsonMixedContractClause, ...]:
    return (
        JsonMixedContractClause(
            pointer="/models",
            expected_json_type="array",
        ),
        JsonMixedContractClause(
            pointer="/models",
            structural_predicate="array_length_gte",
            structural_bound=1,
        ),
        JsonMixedContractClause(
            pointer="/ready",
            scalar_predicate="boolean_is_true",
        ),
        JsonMixedContractClause(
            pointer="/queue_depth",
            scalar_predicate="integer_lte",
            scalar_operand="10",
        ),
    )


def _structural_clauses() -> tuple[JsonMixedContractClause, ...]:
    return (
        JsonMixedContractClause(
            pointer="/models",
            expected_json_type="array",
        ),
        JsonMixedContractClause(
            pointer="/models",
            structural_predicate="array_length_gte",
            structural_bound=1,
        ),
    )


def _claim(
    *,
    count: int | None = 3,
    interval: float | None = 2.0,
    clauses: tuple[JsonMixedContractClause, ...] | None = None,
) -> RealityClaim:
    return RealityClaim.create(
        kind=RealityClaimKind.LOCAL_HTTP_JSON_TIMED_MIXED_CONTRACT,
        statement="The bounded observations satisfy one timed mixed contract.",
        http_url="http://127.0.0.1:11434/api/state",
        expected_http_status=200,
        json_mixed_contract_clauses=(
            clauses if clauses is not None else _mixed_clauses()
        ),
        repeat_observation_count=count,
        minimum_interval_seconds=interval,
    )


def _base_permissions() -> list[str]:
    return [
        "reality.verify",
        "reality.local_http.read",
        "reality.local_http.semantic.read",
        "reality.local_http.repeat.read",
    ]


def _timed_permissions() -> list[str]:
    return _base_permissions() + [
        "reality.local_http.timing.wait",
    ]


def _value_permissions() -> list[str]:
    return _timed_permissions() + [
        "reality.local_http.semantic.value.read",
    ]


def _spine(tmp_path: Path, permissions: list[str]) -> PhiOSSpine:
    return PhiOSSpine(
        state_root=tmp_path,
        allowed_permissions=permissions,
        task_id="timed-mixed-v021",
    )


def _patch_clock(monkeypatch: pytest.MonkeyPatch, clock: FakeClock) -> None:
    monkeypatch.setattr(reality_service, "monotonic", clock.monotonic)
    monkeypatch.setattr(reality_service, "sleep", clock.sleep)


def test_timed_contract_requires_two_to_five_observations() -> None:
    assert (
        "local_http_json_timed_mixed_contract_requires_2_to_5_observations"
        in _claim(count=1).validation_errors()
    )
    assert (
        "local_http_json_timed_mixed_contract_requires_2_to_5_observations"
        in _claim(count=6).validation_errors()
    )
    assert _claim(count=2).validation_errors() == ()
    assert _claim(count=5).validation_errors() == ()


def test_timed_contract_requires_one_to_eight_clauses() -> None:
    empty = _claim(clauses=())
    nine = _claim(
        clauses=tuple(
            JsonMixedContractClause(
                pointer=f"/field{index}",
                expected_json_type="string",
            )
            for index in range(9)
        )
    )

    assert (
        "local_http_json_timed_mixed_contract_requires_1_to_8_clauses"
        in empty.validation_errors()
    )
    assert (
        "local_http_json_timed_mixed_contract_requires_1_to_8_clauses"
        in nine.validation_errors()
    )


def test_timed_interval_is_bounded() -> None:
    error = "local_http_json_timed_mixed_contract_requires_interval_0_05_to_10_seconds"
    assert error in _claim(interval=None).validation_errors()
    assert error in _claim(interval=0.049).validation_errors()
    assert error in _claim(interval=10.001).validation_errors()
    assert error in _claim(interval=True).validation_errors()  # type: ignore[arg-type]
    assert _claim(interval=0.05).validation_errors() == ()
    assert _claim(interval=10.0).validation_errors() == ()


def test_minimum_interval_is_rejected_on_non_timed_claim() -> None:
    claim = RealityClaim.create(
        kind=RealityClaimKind.LOCAL_HTTP_JSON_REPEATED_MIXED_CONTRACT,
        statement="Repeated but not timed.",
        http_url="http://127.0.0.1:11434/api/state",
        expected_http_status=200,
        json_mixed_contract_clauses=_mixed_clauses(),
        repeat_observation_count=3,
        minimum_interval_seconds=1.0,
    )

    assert "minimum_interval_seconds_only_for_timed_mixed_contract" in (
        claim.validation_errors()
    )


def test_timing_wait_grant_is_required_before_io(tmp_path: Path) -> None:
    provider = TimedSequenceProvider(
        clock=None,
        items=[_observation(0), _observation(1), _observation(2)],
    )
    spine = _spine(tmp_path, _base_permissions())

    result = spine.verify_reality(
        claims=(_claim(clauses=_structural_clauses()),),
        local_http_provider=provider,
    )

    assert provider.calls == 0
    assert result.receipt.status is MandalaStatus.BLOCKED
    assert result.claim_results[0]["reason"] == (
        "missing_grant:reality.local_http.timing.wait"
    )


def test_scalar_timed_contract_requires_value_grant_before_io(
    tmp_path: Path,
) -> None:
    provider = TimedSequenceProvider(
        clock=None,
        items=[_observation(0), _observation(1), _observation(2)],
    )
    spine = _spine(tmp_path, _timed_permissions())

    result = spine.verify_reality(
        claims=(_claim(),),
        local_http_provider=provider,
    )

    assert provider.calls == 0
    assert result.receipt.status is MandalaStatus.BLOCKED
    assert result.claim_results[0]["reason"] == (
        "missing_grant:reality.local_http.semantic.value.read"
    )


def test_structural_timed_contract_does_not_require_value_grant(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    clock = FakeClock()
    _patch_clock(monkeypatch, clock)
    provider = TimedSequenceProvider(
        clock=clock,
        items=[_observation(0), _observation(1), _observation(2)],
    )
    spine = _spine(tmp_path, _timed_permissions())

    result = spine.verify_reality(
        claims=(_claim(interval=1.0, clauses=_structural_clauses()),),
        local_http_provider=provider,
    )

    assert provider.calls == 3
    assert result.receipt.status is MandalaStatus.ACCEPTED
    assert result.claim_results[0]["requires_value_read"] is False


def test_monotonic_spacing_applies_between_invocation_starts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    clock = FakeClock()
    _patch_clock(monkeypatch, clock)
    same_wall_clock = "2026-09-18T22:30:00+00:00"
    provider = TimedSequenceProvider(
        clock=clock,
        items=[
            _observation(0, captured_at_utc=same_wall_clock),
            _observation(1, captured_at_utc=same_wall_clock),
            _observation(2, captured_at_utc=same_wall_clock),
        ],
        durations=[0.5, 3.0, 0.0],
    )
    spine = _spine(tmp_path, _value_permissions())

    result = spine.verify_reality(
        claims=(_claim(interval=2.0),),
        local_http_provider=provider,
    )

    assert provider.calls == 3
    assert provider.start_times == [0.0, 2.0, 5.0]
    assert clock.sleeps == [1.5]
    assert result.receipt.status is MandalaStatus.ACCEPTED

    claim_result = result.claim_results[0]
    assert claim_result["interval_basis"] == (
        "provider_invocation_start_monotonic"
    )
    assert claim_result["minimum_interval_seconds"] == 2.0
    assert claim_result["minimum_observed_interval_seconds"] == 2.0
    assert claim_result["timing_satisfied"] is True
    assert [
        item["elapsed_since_previous_start_seconds"]
        for item in claim_result["sample_results"]
    ] == [None, 2.0, 3.0]

    # Deliberately identical wall-clock timestamps do not affect timing proof.
    assert {
        item["captured_at_utc"]
        for item in claim_result["sample_results"]
    } == {same_wall_clock}


def test_timed_contract_attempts_all_samples_after_contradiction_and_timeout(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    clock = FakeClock()
    _patch_clock(monkeypatch, clock)
    provider = TimedSequenceProvider(
        clock=clock,
        items=[
            _observation(0, ready=False),
            LocalHttpObservationError("local_http_timeout", "timed out"),
            _observation(2),
        ],
        durations=[0.0, 0.0, 0.0],
    )
    spine = _spine(tmp_path, _value_permissions())

    result = spine.verify_reality(
        claims=(_claim(interval=1.0),),
        local_http_provider=provider,
    )

    assert provider.calls == 3
    assert provider.start_times == [0.0, 1.0, 2.0]
    assert result.receipt.status is MandalaStatus.DISPUTED
    claim_result = result.claim_results[0]
    assert claim_result["contradicted_count"] == 1
    assert claim_result["unresolved_count"] == 1
    assert claim_result["reason"] == (
        "local_http_json_timed_mixed_sample_contradiction"
    )
    assert claim_result["sample_results"][1]["observation_evidence_ref"] is None


def test_timed_scalar_values_do_not_persist(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    clock = FakeClock()
    _patch_clock(monkeypatch, clock)
    secrets = [781234561, 781234562, 781234563]
    items = [
        _observation(
            index,
            body=json.dumps(
                {
                    "models": [{"name": "alpha"}],
                    "ready": True,
                    "queue_depth": secret,
                },
                separators=(",", ":"),
            ).encode("utf-8"),
        )
        for index, secret in enumerate(secrets)
    ]
    provider = TimedSequenceProvider(
        clock=clock,
        items=items,
    )
    spine = _spine(tmp_path, _value_permissions())
    clauses = (
        JsonMixedContractClause(
            pointer="/models",
            structural_predicate="array_length_eq",
            structural_bound=1,
        ),
        JsonMixedContractClause(
            pointer="/queue_depth",
            scalar_predicate="integer_gte",
            scalar_operand="1",
        ),
    )

    result = spine.verify_reality(
        claims=(_claim(interval=0.5, clauses=clauses),),
        local_http_provider=provider,
    )

    assert result.receipt.status is MandalaStatus.ACCEPTED
    claim_result = result.claim_results[0]
    refs = list(claim_result["sample_evidence_refs"]) + [
        claim_result["series_evidence_ref"]
    ]

    for evidence_ref in refs:
        serialized = spine.soma.evidence.read_bytes(evidence_ref).decode("utf-8")
        assert "observed_value" not in serialized
        for secret in secrets:
            assert str(secret) not in serialized

    series = json.loads(
        spine.soma.evidence.read_bytes(claim_result["series_evidence_ref"])
    )
    assert series["timing_summary"]["timing_satisfied"] is True
    assert series["semantic_contract"]["interval_basis"] == (
        "provider_invocation_start_monotonic"
    )


def test_unresolved_sample_preserves_timing_and_series_is_unknown(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    clock = FakeClock()
    _patch_clock(monkeypatch, clock)
    provider = TimedSequenceProvider(
        clock=clock,
        items=[
            _observation(0),
            LocalHttpObservationError("local_http_timeout", "timed out"),
            _observation(2),
        ],
    )
    spine = _spine(tmp_path, _value_permissions())

    result = spine.verify_reality(
        claims=(_claim(interval=0.5),),
        local_http_provider=provider,
    )

    assert result.receipt.status is MandalaStatus.UNKNOWN
    claim_result = result.claim_results[0]
    assert claim_result["timing_satisfied"] is True
    assert claim_result["supported_count"] == 2
    assert claim_result["unresolved_count"] == 1
    assert [
        item["elapsed_since_previous_start_seconds"]
        for item in claim_result["sample_results"]
    ] == [None, 0.5, 0.5]


class _TimedHandler(BaseHTTPRequestHandler):
    protocol_version = "HTTP/1.1"
    hits = 0

    def do_GET(self) -> None:
        type(self).hits += 1
        body = json.dumps(
            {
                "models": [{"name": "alpha"}],
                "ready": True,
                "queue_depth": type(self).hits,
            },
            separators=(",", ":"),
        ).encode("utf-8")
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self.send_header("Content-Length", str(len(body)))
        self.end_headers()
        self.wfile.write(body)

    def log_message(self, format: str, *args: object) -> None:
        return


@contextmanager
def _loopback_timed_server() -> Iterator[tuple[str, int]]:
    _TimedHandler.hits = 0
    server = ThreadingHTTPServer(("127.0.0.1", 0), _TimedHandler)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        host, port = server.server_address[:2]
        yield str(host), int(port)
    finally:
        server.shutdown()
        server.server_close()
        thread.join(timeout=2.0)


def test_live_timed_contract_uses_requested_count_and_minimum_spacing(
    tmp_path: Path,
) -> None:
    provider = StdlibLoopbackHttpStateProvider()
    spine = _spine(tmp_path, _value_permissions())
    clauses = (
        JsonMixedContractClause(
            pointer="/models",
            structural_predicate="array_length_gte",
            structural_bound=1,
        ),
        JsonMixedContractClause(
            pointer="/ready",
            scalar_predicate="boolean_is_true",
        ),
        JsonMixedContractClause(
            pointer="/queue_depth",
            scalar_predicate="integer_lte",
            scalar_operand="10",
        ),
    )

    with _loopback_timed_server() as (host, port):
        claim = RealityClaim.create(
            kind=RealityClaimKind.LOCAL_HTTP_JSON_TIMED_MIXED_CONTRACT,
            statement="Three timed local observations satisfy the contract.",
            http_url=f"http://{host}:{port}/api/state",
            expected_http_status=200,
            json_mixed_contract_clauses=clauses,
            repeat_observation_count=3,
            minimum_interval_seconds=0.05,
        )
        result = spine.verify_reality(
            claims=(claim,),
            local_http_provider=provider,
        )
        hits = _TimedHandler.hits

    assert hits == 3
    assert result.receipt.status is MandalaStatus.ACCEPTED
    claim_result = result.claim_results[0]
    assert claim_result["timing_satisfied"] is True
    assert claim_result["minimum_observed_interval_seconds"] >= 0.05
    assert claim_result["supported_count"] == 3
    assert "timed_http_wait_requires_separate_grant" in result.receipt.limitations
    assert "timed_mixed_success_is_not_continuous_health" in (
        result.receipt.limitations
    )
